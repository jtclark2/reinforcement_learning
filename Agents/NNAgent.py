"""
This is a direct copy of my homework Course4, assignment2 in the RL specialization from Coursera. The intent is to get
this working and then adjustment until I understand this thing well enough to integrate it into the rest of the system.
"""
...
class ActionValueNetwork:
    ...
    def get_action_values(self, s):
        ...
        psi = np.dot(s, W0) + b0  # Linear vector result of layer 0 (not activated yet)
        x = np.maximum(psi, 0)  # activation of layer 0

        W1, b1 = self.weights[1]['W'], self.weights[1]['b']  # weights of layer 1
        q_vals = np.dot(x,
                        W1) + b1  # linear combination of layer 1...I guess we're not applying softmax activation? Must be done in a later step
    ...
    def get_TD_update(self, s, delta_mat):
        """
        Args:
            s (Numpy array): The state. ???Also batch (batch_size,state_size)???
        ...
        psi = np.dot(s, W0) + b0  # forward prop of layer 0
        x = np.maximum(psi, 0)  # activation of layer 0
        ...
        v = delta_mat  # output label...I think?
        td_update[1]['W'] = np.dot(x.T, v) * 1. / s.shape[0]  # backprop into layer 1
        td_update[1]['b'] = np.sum(v, axis=0, keepdims=True) * 1. / s.shape[0]  # backprop into layer 1

        v = np.dot(v, W1.T) * dx  # backprop of the layer 0 activation
        td_update[0]['W'] = np.dot(s.T, v) * 1. / s.shape[0]  # backprop into layer 0
        td_update[0]['b'] = np.sum(v, axis=0, keepdims=True) * 1. / s.shape[0]  # backprop into layer 0
    ...
    def update_weights(self, weights, td_errors_times_gradients):
        ...
        for i in range(len(weights)):
            for param in weights[i].keys():
                ...
                weight_update = self.step_size / (v_hat ** 0.5 + self.epsilon) * m_hat
                ### END CODE HERE
...
def softmax(action_values, tau=1.0):
    ...
    sum_of_exp_preferences = np.sum(exp_preferences, axis=1)  # 1 to sum over actions dimension
...
def get_td_error(states, next_states, actions, rewards, discount, terminals, network, current_q, tau):
    ...
    v_next_vec = np.sum(q_next_mat * probs_mat, 1) * (
                1 - terminals)  # correct dimension on the sum? [batch_size,state_dim]
    ### END CODE HERE
    ...
    ### START CODE HERE (~1 Line)
    # I still need to re-read notes about this 2 network system...I get using previous states, but why are 2 networks needed?
    # Will we ocasionally copy network = deepcopy(current_q), and why not use clearer names?
    q_mat = network.get_action_values(states)  # I call prev, but they call current
    ### END CODE HERE
    ...
    ### START CODE HERE (~1 Line)
    # Can you use [:,index] insted of [batch_indeces, index]?
    # I tried it...the results match, but shape does not:
    # The [batch_indices,actions method yields [batch_size]
    # My [:,action] yields [[batch_size]]
    # And using [batch_indices][actions] yields [batch_size, actions], and every action is identical
    q_vec = q_mat[batch_indices, actions]  # actions shape is (batch_size,)
    ### END CODE HERE
...
class Agent: # (BaseAgent):
    def __init__(self, agent_config=None):
        ...
        if agent_config is None:
            agent_config = {
                ...
                'minibatch_sz': 32, # originally 8, I increased
    ...
    def load_agent_memory(self, load_path):
        print("Warning: not checking if function approximator matches...we may clobber some stuff.") # TODO
        (self.network, self.replay_buffer) = pickle.load(open(load_path, "rb"))
        return True
...
if __name__ == "__main__":
    ...
    agent = NNAgent.Agent() # no args --> default params
    ...
    trainer.run_multiple_episodes(total_steps,
                                  render_interval,
                                  frame_delay,
                                  history_file_path=trainer_file_path,
                                  save=save)
